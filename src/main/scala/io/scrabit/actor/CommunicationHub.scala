...
object CommunicationHub:
  ...
  case class UserSession(userId: String,
                         sessionKey: String,
                         connection: ActorRef[OutgoingMessage],
                         roomId: Option[Int]) {
  ...
  case class CreateRoom(owner: String, behavior: Behavior[RoomMessage], replyTo: ActorRef[RoomMessage] /*Room Supervisor message ?*/) extends InternalMessage
  ...
  case class Data(users: Map[String, UserSession],
    ...
    def deleteSession(userSession: UserSession): Data = {
        this.copy(users = users - userSession.userId,
                  sessions = sessions - userSession.sessionKey,
                  connections = connections - userSession.connection)
    }
   ...
   def create(authenticationService: ActorRef[AuthenticationService.Login]): Behavior[Message] = Behaviors.setup { context =>
     ...
     Actor(authenticationService)(Data.empty)
   }
  

  private class Actor(val authenticationService: ActorRef[AuthenticationService.Login]) {
       def apply(data: Data): Behavior[Message] = Behaviors.receive[Message] { (context, message) =>
         message match {
           ...
           case CreateRoom(owner, behavior, replyTo) =>
             ...
             data.addRoom(spawnActor) match {
               ...
               case Some((updatedData, roomId, ref)) => 
                 replyTo ! RoomMessage.RoomCreated(owner, roomId, ref)
           ...
           case req: Request =>
             data.users.get(req.userId) match {
               case None =>
                 context.log.warn(s"userId ${req.userId} not exist")
               case Some(session) =>
                 session.roomId foreach { roomId =>
                   data.rooms(roomId) ! req.toRoomMessage
                 }
             }
             Behaviors.same
